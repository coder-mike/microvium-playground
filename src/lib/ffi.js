...
const virtualMemberDeclarations = [];
...
${args.length ? `${join(map(args, (a, i) => `  _args[${i}] = ${emitIncomingConversion(a[0], a[1])};`), '\n')}\n` : ''}\
  mvm_Value _result;
...
export function importFromC(returnType, name, args) {
  const cSignature = `${typeToC(returnType)} ${name}(${join(map(args, emitArg), ', ')})`;
  const virtualMember = `\n  ${cSignature};`
  virtualMemberDeclarations.push(virtualMember);

  const wrapperName = `_${name}_wrapper`;
  const importInfo = getNextImportID(wrapperName);
  internalHeaders.push(`static mvm_TeError ${wrapperName}(mvm_VM* _vm, mvm_HostFunctionID hostFunctionID, mvm_Value* _result, mvm_Value* _args, uint8_t _argCount);`);

  const implementation = appName => `
mvm_TeError ${appName}::${wrapperName}(mvm_VM* _vm, mvm_HostFunctionID hostFunctionID, mvm_Value* _result, mvm_Value* _args, uint8_t _argCount) {
  ...
  // Call the C function
  ${returnType === 'void' ? '' : `${typeToC(returnType)} __result = `}_app->${name}(${join(map(args, a => a[1]), ', ')});
...
public:
  ...
  ${appName}(const std::vector<uint8_t>& snapshot);
...
protected:
  ...
  ${join(virtualMemberDeclarations, '')}
...
${!imports.length ? '' : `
const ImportRecord ${appName}::_vmImports[${imports.length}] = {
  ${join(map(imports, (im, i) => `/*[${i}]*/ { ${im.id}, ${appName}::${im.name} },`), '\n  ')}
};`
}
...
void ${appName}::_initialize(const uint8_t* snapshot, size_t snapshotSize) {
  mvm_TeError err;
...
${appName}::${appName}(const std::vector<uint8_t>& snapshot): _snapshotCopy(snapshot) {
  _initialize(_snapshotCopy.data(), _snapshotCopy.size());
}

mvm_TeError ${appName}::_resolveImport(mvm_HostFunctionID hostFunctionID, void* context, mvm_TfHostFunction* out_hostFunction) {
...
`
  ...
  fs.writeFileSync(path.join(outDir, ffiHeaderName), header);
  fs.writeFileSync(path.join(outDir, ffiImplementationName), implementation);
}
